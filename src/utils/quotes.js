  // Cycle through quotes every 5 seconds with fade transition

export const QUOTES = [
  {
    q: "Believe you can and you're halfway there.",
    a: "Theodore Roosevelt",
  },
  {
    q: "Act as if what you do makes a difference. It does.",
    a: "William James",
  },
  {
    q: "Success is not final, failure is not fatal: It is the courage to continue that counts.",
    a: "Winston Churchill",
  },
  {
    q: "It is never too late to be what you might have been.",
    a: "George Eliot",
  },
  {
    q: "Focus is a matter of deciding what things you're not going to do.",
    a: "John Carmack",
  },
  {
    q: "Fear can hold you prisoner. Hope can set you free.",
    a: "Stephen King",
  },
  {
    q: "Working hard for something we don't care about is called stress: Working hard for something we love is called passion.",
    a: "Simon Sinek",
  },
  {
    q: "A very little key will open a very heavy door.",
    a: "Charles Dickens",
  },
  {
    q: "Give more than you think you can, trusting that you are richer than you think.",
    a: "Jon Kabat-Zinn",
  },
  {
    q: "Ideas do not reach perfection in a day, no matter how much study is put upon them.",
    a: "Alexander Graham Bell",
  },
  {
    q: "Life is either a daring adventure, or it is nothing.",
    a: "Helen Keller",
  },
  {
    q: "Give a man a fish and you feed him for a day. Teach a man to fish and you feed him for a lifetime.",
    a: "Lao Tzu",
  },
  {
    q: "What you become is far more important than what you get.",
    a: "Jim Rohn",
  },
  {
    q: "Whatever your mind can conceive and believe, it can achieve.",
    a: "Napoleon Hill",
  },
  {
    q: "Love never keeps a man from pursuing his destiny.",
    a: "Paulo Coelho",
  },
  {
    q: "Winning isn't everything, but wanting to win is.",
    a: "Vince Lombardi",
  },
  {
    q: "The pursuit of mastery bears gifts.",
    a: "Gary Keller",
  },
  {
    q: "Everything that is made beautiful and fair and lovely is made for the eye of one who sees.",
    a: "Rumi",
  },
  {
    q: "The key to success is to focus our conscious mind on things we desire not things we fear.",
    a: "Brian Tracy",
  },
  {
    q: "Great things are not done by impulse, but by a series of small things brought together.",
    a: "Vincent van Gogh",
  },
  {
    q: "He who fails to plan, plans to fail.",
    a: "Unknown",
  },
  {
    q: "It is amazing what you can accomplish if you do not care who gets the credit.",
    a: "Harry S. Truman",
  },
  {
    q: "A little progress each day adds up to big results.",
    a: "Unknown",
  },
  {
    q: "Our greatest weakness lies in giving up. The most certain way to succeed is always to try just one more time.",
    a: "Thomas Edison",
  },
  {
    q: "Learning is a weightless treasure you can always carry easily.",
    a: "Chinese Proverb",
  },
  { q: "Fall seven times, stand up eight.", a: "Japanese Proverb" },
  { q: "It always seems impossible until it's done.", a: "Nelson Mandela" },
  { q: "The harder the battle, the sweeter the victory.", a: "Les Brown" },
  {
    q: "Difficulties strengthen the mind, as labor does the body.",
    a: "Seneca",
  },
  {
    q: "A river cuts through rock not because of its power, but because of its persistence.",
    a: "James N. Watkins",
  },
  {
    q: "Failure is simply the opportunity to begin again, this time more intelligently.",
    a: "Henry Ford",
  },
  {
    q: "Perseverance is not a long race; it is many short races one after another.",
    a: "Walter Elliot",
  },
  {
    q: "Courage doesn't always roar. Sometimes it's the quiet voice at the end of the day saying, 'I will try again tomorrow.'",
    a: "Mary Anne Radmacher",
  },
  { q: "Once you stop learning, you start dying.", a: "Albert Einstein" },
  {
    q: "Don't be afraid to give up the good to go for the great.",
    a: "John D. Rockefeller",
  },
  { q: "Growth and comfort do not coexist.", a: "Ginni Rometty" },
  { q: "The expert in anything was once a beginner.", a: "Helen Hayes" },
  { q: "Mistakes are proof that you are trying.", a: "Jennifer Lim" },
  { q: "You don't have to be perfect to be amazing.", a: "Unknown" },
  {
    q: "The best way to predict the future is to create it.",
    a: "Peter Drucker",
  },
  {
    q: "Be not afraid of growing slowly; be afraid only of standing still.",
    a: "Chinese Proverb",
  },
  {
    q: "Every accomplishment starts with the decision to try.",
    a: "John F. Kennedy",
  },
  { q: "Small steps every day lead to big changes.", a: "Unknown" },
  { q: "Once you stop learning, you start dying.", a: "Albert Einstein" },
  {
    q: "Don't be afraid to give up the good to go for the great.",
    a: "John D. Rockefeller",
  },
  { q: "Growth and comfort do not coexist.", a: "Ginni Rometty" },
  { q: "The expert in anything was once a beginner.", a: "Helen Hayes" },
  { q: "Mistakes are proof that you are trying.", a: "Jennifer Lim" },
  { q: "You don't have to be perfect to be amazing.", a: "Unknown" },
  {
    q: "The best way to predict the future is to create it.",
    a: "Peter Drucker",
  },
  {
    q: "Be not afraid of growing slowly; be afraid only of standing still.",
    a: "Chinese Proverb",
  },
  {
    q: "Every accomplishment starts with the decision to try.",
    a: "John F. Kennedy",
  },
  { q: "Small steps every day lead to big changes.", a: "Unknown" },
  {
    q: "Start where you are. Use what you have. Do what you can.",
    a: "Arthur Ashe",
  },
  { q: "The secret of getting ahead is getting started.", a: "Mark Twain" },
  { q: "Dream big. Start small. Act now.", a: "Robin Sharma" },
  {
    q: "Don't watch the clock; do what it does. Keep going.",
    a: "Sam Levenson",
  },
  { q: "You miss 100% of the shots you don't take.", a: "Wayne Gretzky" },
  {
    q: "Do something today that your future self will thank you for.",
    a: "Sean Patrick Flanery",
  },
  { q: "If you get tired, learn to rest, not to quit.", a: "Banksy" },
  { q: "Never let your fear decide your fate.", a: "AWOLNATION" },
  {
    q: "Start where you are. Use what you have. Do what you can.",
    a: "Arthur Ashe",
  },
  { q: "The secret of getting ahead is getting started.", a: "Mark Twain" },
  { q: "Dream big. Start small. Act now.", a: "Robin Sharma" },
  {
    q: "Don't watch the clock; do what it does. Keep going.",
    a: "Sam Levenson",
  },
  { q: "You miss 100% of the shots you don't take.", a: "Wayne Gretzky" },
  {
    q: "Do something today that your future self will thank you for.",
    a: "Sean Patrick Flanery",
  },
  { q: "If you get tired, learn to rest, not to quit.", a: "Banksy" },
  { q: "Never let your fear decide your fate.", a: "AWOLNATION" },
  {
    q: "The only way to do great work is to love what you do.",
    a: "Steve Jobs",
  },
  {
    q: "Don't aim for success if you want it; just do what you love and believe in.",
    a: "David Frost",
  },
  {
    q: "If you can't figure out your purpose, figure out your passion. For your passion will lead you right into your purpose.",
    a: "T.D. Jakes",
  },
  {
    q: "What lies behind us and what lies before us are tiny matters compared to what lies within us.",
    a: "Ralph Waldo Emerson",
  },
  {
    q: "To be yourself in a world that is constantly trying to make you something else is the greatest accomplishment.",
    a: "Ralph Waldo Emerson",
  },
];
